...
class Asset(object):
	...
	'''
	@classmethod
	def update(cls,warranty,cpu,ram,disk,idc_id,id):
		_sql = 'update assets set warranty=%s,cpu=%s,ram=%s,disk=%s,idc_id=%s where id =%s'
		_args = (warranty,cpu,ram,disk,idc_id,id)
		MySQLConnection.execute_sql(_sql,_args,fetch=False)
