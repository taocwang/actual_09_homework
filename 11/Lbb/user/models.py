...
class DbBase(object):
    ...
    def get_list(cls, wheres=[]):
        ...
        # print [dict(zip(cls.columns_select, _line)) for _line in _rt_list]
...
class User(DbBase):
    ...
    def add(cls,username,password,age):
        ...
        _args = (username, cls.md5_str(password), age)
    ...
    def validate_login(cls, username, password):
        ...
        _count, _rt_list = MySQLConnection.execute_sql(_sql, (username, cls.md5_str(password)))
...
class AccessLog(object):
    ...
    @classmethod
    def statis_get_list(cls):
        _sql = 'select code,sum(cnt) from accesslog group by code'
        _count,_rt_list = MySQLConnection.execute_sql(_sql)
        _dict = dict(_rt_list)
        _rt = [str(key) for key in _dict]
        _sum = sum([_dict.get(key) for key in _dict])
        _float = map(lambda x:round(float(x)/float(_sum)*100,4),[float(_dict.get(key)) for key in _dict])
        return zip(_rt,_float)
...
class Performs(object):
    ...
    @classmethod
    def get_rate_list(cls,ip):
        _sql = 'select cpu,ram FROM performs where ip=%s and time >= %s order by time asc'
        _args = (ip,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time() - 60)))
        _count,_rt_list = MySQLConnection.execute_sql(_sql,_args)
        cpu_list = []
        ram_list = []
        for _cpu,_ram in _rt_list:
            cpu_list.append(_cpu)
            ram_list.append(_ram)
        print cpu_list,ram_list
        return cpu_list,ram_list


class Command(DbBase):
    @classmethod
    def validate(cls,req,musername):
        _is_ok = True
        _error = {}
        if Asset.get_by_key(req.get('id')) is None:
            _is_ok = False
            _error['id'] = '资产不存在'
        elif req.get('username').strip() == '':
            _is_ok = False
            _error['username'] = '系统用户名为空'
        elif req.get('password').strip() == '':
            _is_ok = False
            _error['password'] = '系统密码为空'
        elif req.get('admin-password').strip() == '':
            _is_ok = False
            _error['admin-password'] = '管理员密码为空'
        elif not User.validate_login(musername,req.get('admin-password')):
            _is_ok = False
            _error['password'] = '管理员密码错误'
        elif req.get('cmds').strip() == '':
            _is_ok = False
            _error['cmds'] = '命令为空'
        return _is_ok,_error
    ...
    def execute(cls,req):
        ...
        _is_ok,_result = ssh.ssh_execute(_asset['ip'],_username,_password,_cmds)
        _echos = []
        #print _is_ok
        if _is_ok:
            for _cmd,_out,_errs in _result:
                print _cmd,_out,_errs
                _echos.append(_cmd)
                _echos.append(''.join(_out))
                _echos.append(''.join(_errs))
            #print _echos
            return True,'\n'.join(_echos)
        #print '\n'.join(_result)
        return False,'\n'.join(_result)
...
if __name__ == '__main__':
    ...
    print AccessLog.statis_get_list()
    print Performs.get_rate_list('10.0.2.15')
