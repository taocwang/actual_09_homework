...
@app.route('/user/chargpasswd/',methods=['POST'])
@checkinfo
def user_chargpasswd():
    import userdb
    admin_name=session.get('user')['username']
    admin_passwd=request.form.get('admin_passwd')
    user_name=request.form.get('username')
    user_passwd=request.form.get('password')
    _is_ok,_error=userdb.valid_user(user_name,user_passwd,admin_name,admin_passwd)
    if _is_ok:
        import usermodify
        usermodify.chargpasswd(user_name,user_passwd)
