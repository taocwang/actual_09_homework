...
import time
from models import User,Performs
...
def update_assets():
	...
	print 'i am id--%s' % _id
	_assets = Asset.get_by_id(_id)

	return render_template('asset_update.html',idcs=_idcs,asset=_assets)
...
def addupdate_assets():
	...
	if _is_ok:
		Asset.update(idc_id=_idc_id,warranty=_warranty,cpu=_cpu,ram=_ram,disk=_disk,id=_id,ip=_ip)
	return json.dumps({'is_ok':_is_ok,'error':_error})
...
@app.route('/asset/cmd/',methods=['GET','POST'])
def asset_cmd():
	_host = request.form.get('host')
	_port = int(request.form.get('port'))
	_username = request.form.get('username')
	_password = request.form.get('password')
	_cmd = request.form.get('cmd')

	_commands = ssh.ssh_command(_host,_port,_username,_password,_cmd)
	print _commands

	return json.dumps({'command':_commands})
