...
def create():
    return render_template('adduser.html', info=u'请按顺序填写：用户名、年龄、密码')
...
def addUser():
    ...
    if user.validate_find(username):
        # print '用户存在'
        return render_template('adduser.html', error=u'用户名存在，请重新添加')
    elif not username or not age or not password:
        return render_template('adduser.html', error=u'(用户名、年龄、密码)不能为空，请重新添加')
    else:
        # print '用户不存在, 请编写添加代码块'
        if user.add_users(username=username, age=int(age), password=password):
            return redirect('/users/')
        else:
            print '用户添加失败！'
...
def update():
    ...
    if not username or not age or not password:
        return render_template('updateUser.html', error=u'(年龄、密码)不能为空，请重新添加')
...
功能ok，加油
改进点
1. 修改用户信息，可以将用户信息查询后展示到修改的表单内 line 89添加查询用户信息
2. 删除可以不用到新的页面，直接删除返回到/users/
3. 增、删、改都有修改users.json的功能，能否用统一的函数处理
'''
