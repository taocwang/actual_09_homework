...
class Asset(DbBase):
    ...
    @classmethod
    def validate_cmd(cls,id,musername,mpassword,cmd):
        _is_ok = True
        _error = {}
        if cls.get_by_key(id) is None:
            _is_ok = False
            _error['id'] = '用户不存在'
        elif len(mpassword) < 0:
            _is_ok = False
            _error['mpassword'] = '管理员密码为空'
        elif not User.validate_login(musername,mpassword):
            _is_ok = False
            _error['mpassword'] = '管理员密码错误'
        elif cmd.strip() == '':
            _is_ok = False
            _error['cmd'] = '命令为空'
    ...
    def cmd(cls,ip,cmds):
        ...
        try:
            ...
            ssh.connect(ip,22,'root','1qaz@WSX')
            for _cmd in cmds:
                stdin,stdout,stderr = ssh.exec_command(_cmd)
                _rt_list.append([_cmd,stdout.readlines(),stderr.readlines()])
            for _cmd,_output,_err in _rt_list:
                _rt_dict['command']=_cmd
                _rt_dict['cmd_result']=_output
                _rt_dict['err_msg']=_err
                _rt.append(_rt_dict)
            return _rt
        except:
            #[{'err_msg': ['用户连结失败请检查端口和ip以及用户名密码'],'cmd_result': ['fail']}]
            return [{'err_msg': ['用户连结失败请检查端口和ip以及用户名密码'],'cmd_result': ['fail']}]
        finally:
            if ssh:
                ssh.close()
...
if __name__ == '__main__':
    ...
    print Asset.cmd('182.61.42.4',['id'])