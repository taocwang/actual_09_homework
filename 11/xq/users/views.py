...
from modules import User,Assets,Performs,Remote_cmd,Center                  #Modules修改成类的形式
import os,json
...
@app.route('/monitor_resource/',methods=['POST'])
...
@app.route('/asserts/cmd/')
def get_cmd():
    return render_template('exec_cmd.html')


@app.route('/asserts/execcmd/',methods=['GET','POST'])
def exec_cmd():
    _cmds = []
    _rrt = {}
    port=21860
