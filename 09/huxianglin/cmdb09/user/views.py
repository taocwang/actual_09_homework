...
def assets():
    ...
    if msg:
        return render_template('asset.html',assets=_assets,username=session.get('username',''),privilege=session.get('privilege',''),color=color,msg=msg)
    # for i in range(0,len(_assets)):
    #     _assets[i]['purchase_date']=_assets[i]['purchase_date'].strftime("%Y-%m-%d")
    return render_template('asset.html',assets=_assets,username=session.get('username',''),privilege=session.get('privilege',''))

@app.route('/asset/create/',methods=['POST','GET'])
#@login_required
def create_asset():
    _idcs = [('1', u'北京-亦庄'), ('2', u'北京-酒仙桥'), ('3', u'北京-西单'), ('4', u'北京-东单')]
...
def add_asset():
    ...
    if Flag:
        return json.dumps({'_is_ok':True,'errors':{'successful':u'数据插入成功！'}})
    else:
...
def modify_asset():
    ...
    _assets=asset.get_list()
    _idcs = [('1', u'北京-亦庄'), ('2', u'北京-酒仙桥'), ('3', u'北京-西单'), ('4', u'北京-东单')]
    return render_template('asset_modify.html',idcs=_idcs,asset=_assets[index-1])

@app.route('/asset/change/',methods=['POST'])
#@login_required
def change_asset():
    params=request.form
    ...
    if not Flag:
        ...
        if Flag:
            return json.dumps({'_is_ok':True,'errors':{'successful':u'数据更新成功！'}})
        else:
            return json.dumps({'_is_ok':False,'errors':{'error':u'数据更新失败！'}})

@app.route('/asset/delete/',methods=['GET'])
#@login_required
def delete_asset():
    id=request.args.get('id','')
    Flag=asset.delete(id)
    if Flag:
        return redirect(url_for('assets',color='alert-success',msg=u'删除成功'))
    else:
