...
返回值: (True/False, 错误信息)
'''
def validate_add_user(s_username,username, password, age):
    #处理密码，判断是否有大写、小写和数字
...
增加同名用户判断，把ID加进去
'''
def validate_update_user(id,s_id,username,s_username,password, age):
    _up_pass = [x for x in password if x.isupper()]
    _low_pass = [x for x in password if x.islower()]
    _isdigit_pass = [x for x in password if x.isdigit()]
    if s_username!='admin':
...
def upload_validate_check(path):
...
def validate_charge_user_password(uid, upassword, musername, mpassword):
    ...
    #只有管理员才能修改其他用户密码
    if not check_is_admin(musername):
