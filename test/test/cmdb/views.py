...
def users():
    ...
    return render_template('users.html', users=_users, username=session.get('user').get('username'), msg=request.args.get('msg', ''), userjson=json.dumps(_users))            #加载渲染users.html模板
...
@login_required
def create_user():
    return render_template('user_create.html', idcs=[(1, u'北京'), (2, u'上海'), (3, u'陕西')])      #加载渲染user_create.html
...
@app.route('/user/modify-password/')
@login_required
def modify_user_password():
    ...
    return render_template('password_modify.html', error=_error, username=_username, uid=_uid)
