...
class Performs(object):
	...
	def add(cls, req):
		...
		_ram = req.get('ram')
		_time = req.get('time')
		_args = (_ip, _cpu, _ram, _time)
		_fetch = False
		_sql = 'insert into performs (ip, cpu, ram, time) values(%s, %s, %s, %s)';
		count, rt_list = MySQLConnection.execute_sql(_sql, _args, _fetch)		
	...
	def get_list(cls,ip):
		...
		count, rt_list = MySQLConnection.execute_sql(_sql,_args)
		_datetime_list=[]
		_cpu_list=[]
		_ram_list=[]
		for _cpu, _ram, _time in rt_list:
			_cpu_list.append(_cpu)
			_ram_list.append(_ram)
			_datetime_list.append(_time.strftime('%H:%M:%S'))
		return _datetime_list, _cpu_list, _ram_list
			
if __name__ == '__main__':
	print Performs.get_list('192.168.137.2')