...
'''
def get_info(_sql=None,_count=None,_user=None,id=None,username=None,_args=()):
...
def validate_update_user(id,username, password, age):
    #从数据库中提取用户信息，并判断当前post的用户名是否和数据库中有重复？判断条件为：ID不同
    _users = get_user(id=id)
    _username = _users.get('username')
    _id = _users.get('id')
...
'''更新用户信息password=md5(%s),password=md5(password)
