...
def useredit():
	...
	if user_id and useredit_age and useredit_gender and useredit_email:
		if User.update(age=useredit_age, user_id=user_id, gender=useredit_gender, email=useredit_email):
			flash('恭喜您,用户信息更改成功')
			_is_ok = True
			return json.dumps({'is_ok':_is_ok})
		else:
...
@app.route('/asset/performs/', methods=['POST','GET'])	
@login_required
def perform_asset():
	asset_id = request.form.get("asset_id")
	_ip = Asset.get_list(asset_id)[0]['ip']
	datetime_list, cpu_list, ram_list = Performs.get_list(_ip)
	return render_template('asset_perform.html', datetime_list=json.dumps(datetime_list), cpu_list=json.dumps(cpu_list), ram_list=json.dumps(ram_list))
