...
def users():
    ...
    #print _users
    #获取当前用户的信息
    _user,_error = user.get_info(_id=session.get('id').get('id'))
    #print _user
...
def get_asserts():
    #获取所有资产的信息
    _asserts = asserts.get_list()
    return render_template('asserts.html',asserts=_asserts)
...
def add_asserts():
    ...
    print _cpu
    _is_ok, _error = asserts.validate_create(_sn,_ip,_hostname,_os,_cpu,_ram,_disk,_idc_id,_admin,_business,_purchase_date,_warranty,_vendor,_model)
    if _is_ok:
        asserts.create(_sn,_ip,_hostname,_os,_cpu,_ram,_disk,_idc_id,_admin,_business,_purchase_date,_warranty,_vendor,_model)
    return json.dumps({'is_ok':_is_ok, "error":_error})
...
def delete_asserts():
    _id = request.args.get('id')
    _asset,_error = asserts.get_by_id(_id=_id)
    if not _asset:
        _error = '资产不存在'
    else:
        _session_username = session['user']['username']
...
def modify_asserts():
    ...
    _is_ok, _error = asserts.validate_update(_sn,_ip,_hostname,_os,_cpu,_ram,_disk,_idc_id,_admin,_business,_purchase_date,_warranty,_vendor,_model)
    print _is_ok
    if _is_ok:
        #print "hello"
        asserts.update(_sn,_ip,_hostname,_os,_cpu,_ram,_disk,_idc_id,_admin,_business,_purchase_date,_warranty,_vendor,_model,_id)
    return json.dumps({'is_ok':_is_ok, "error":_error})
...
def get_asset_by_id(id):
    _rt,_error = asserts.get_by_id(id)
