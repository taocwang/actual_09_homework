...
def cmd_asset():
    ...
    return render_template('asset_cmd.html',asset=_asset)


@app.route('/asset/cmd_carry/',methods=['POST'])
def cmd_carry_asset():
    _id = request.form.get('id')
    _ip = request.form.get('ip')
    _mpassword = request.form.get('mpassword')
    _cmd = request.form.get('cmd')
    _is_ok,_error = Asset.validate_cmd(_id,session['user']['username'],_mpassword,_cmd)
    if _is_ok:
       _cmd_result =  Asset.cmd(_ip,[_cmd])
       print _cmd_result
       _er = _cmd_result[0].get('err_msg')
       _rs = _cmd_result[0].get('cmd_result')
       print len(_er) == 0,_rs
       if len(_er) != 0:
            return json.dumps({'is_ok' : _is_ok, 'errors' : _error,'rst':_er})
       else:
            return json.dumps({'is_ok' : _is_ok, 'errors' : _error,'rst':_rs})
    return json.dumps({'is_ok' : _is_ok, 'errors' : _error})
