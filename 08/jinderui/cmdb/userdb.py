...
def change_user(userid,updateage):
...
#修改密码进行验证
def validate_charge_user_password(userid,upassword,musername,mpassword):
...
def charge_user_password(userid,upassword):
	_sql = 'update user set password=md5(%s) where id=%s'
	execute_commit_sql(_sql,(upassword,userid))
