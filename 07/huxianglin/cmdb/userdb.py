...
def GetUsers():
    ...
    _colums=('id','username','password','age','privilege')
...
def AddUser(username,password,age,privilege):
    _sql='insert into user(username,password,age,privilege) values(%s,md5(%s),%s,%s)'
    _args=(username,password,age,privilege)
...
def ChangeUser(id,username,password,age,privilege):
    ...
    if _count:
        ...
        _sql='update user set username=%s,password=md5(%s),age=%s,privilege=%s where id=%s'
        _args=(username,password,age,privilege,id)
        _count=dbutil.execute_commit_sql(_sql,_args)
        if _count:
            return True
        else:
